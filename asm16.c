...
struct {
	...
	void (*func)( label*, char** );
		...
		"ALIGN",align,
		"MACRO",macro,
		"REPT",rept,"REPEAT",rept,
		"ENDM",endm,"ENDMACRO",endm,
		"ENDR",endr,"ENDREPEAT",endr,
		"ENUM",_enum,
		"ENDE",ende,"ENDENUM",ende,
		"FILLVALUE",fillval,
		"DL",dl,
		"DH",dh,
		"FDB",fdb,
...
void tw(label *id, char **next) {
	// take in one word, one string.
	// for each character in string, OR it
	// with the word.
	int val,val2;
	byte *s,*start;
	char c,quote;
	int orval=eval(next,WHOLEEXP);
	if(orval<0||val>0xFFFF)
	  errmsg=OutOfRange;
	eatchar(next,',');
	*next+=strspn(*next,whitesp);	   //eatwhitespace
	quote=**next;
