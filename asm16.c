...
byte asl[]={0x0a,ACC,0x16,ZPX,0x1e,ABSX,0x06,ZP,0x0e,ABS,0x0e,ABSL,0x0a,IMP,-1};
byte lsr[]={0x4a,ACC,0x56,ZPX,0x5e,ABSX,0x46,ZP,0x4e,ABS,0x4E,ABSL,0x4a,IMP,-1};
byte rol[]={0x2a,ACC,0x36,ZPX,0x3e,ABSX,0x26,ZP,0x2e,ABS,0x2E,ABSL,0x2a,IMP,-1};
byte ror[]={0x6a,ACC,0x76,ZPX,0x7e,ABSX,0x66,ZP,0x6e,ABS,0x6E,ABSL,0x6a,IMP,-1};
byte ora[]={0x13,RELSY,0x03,RELS,0x17,ZPILY,0x12,IND,
	0x09,IMML,0x09,IMM,0x01,INDX,0x11,INDY,0x15,ZPX,0x1d,ABSLX,0x19,ABSX,0x1F,ABSY,0x05,ZP,0x0d,ABS,0x0F,ABSL,-1};
byte jsr[]={0x22,ABSL,//alias jsl
	0x20,ABS,-1};
byte and[]={0x33,RELSY,0x23,RELS,
	0x29,IMML,0x29,IMM,0x21,INDX,0x31,INDY,0x35,ZPX,0x3F,ABSLX,0x3d,ABSX,0x39,ABSY,0x25,ZP,0x2d,ABS,0x2F,ABSL,-1};
...
byte sta[]={0x83,RELS,0x93,RELSY,
	0x81,INDX,0x91,INDY,0x95,ZPX,0x9F,ABSLX,0x9d,ABSX,0x99,ABSY,0x85,ZP,0x8d,ABS,0x8F,ABSL,-1};
byte sty[]={
...
void opcode(label *id, char **next) {
    ...
    for(op=(byte*)(*id).line;*op!=0xff;op+=2) {//loop through all addressing modes for this instruction
        ...
        if(type!=IMP && type!=ACC) {//get operand
            ...
            if(type==REL) {
                ...
                if(opsize[type]==1) {
					...
					if ((val<-(0xffffff/2) || val>0xffffff) && !dependant)
						errmsg=OutOfRange;
					else {
						if(((val & 0xFFFF) == val) && !dependant && (type==ABSLX || type==IMML))
							continue; // we don't need a long opcode
					}
				}
            }
        ...
        //dumb workaround...
        if (!strcmp((*id).name, "SBC") && type==ABSLX) {
            output_le(0xFF,1);
        } else {
            output(op,1);
        }
